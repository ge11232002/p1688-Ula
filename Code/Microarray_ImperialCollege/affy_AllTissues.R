...
infGenes <- read_xlsx(here("Data", "Inflammasomes_and_IL1signaling_gene_list_v2_280217.xlsx"))
infGenes <- infGenes$`Gene Symbol`

# grouping of samples
grouping <- list("Healthy"=c("DJ116","DJ114","DJ47","DJ101","DJ56","DJ48","DJ81"),
                 "AsthmaHDM"=c("DJ144","DJ129","DJ121","DJ139","DJ92","DJ148","DJ67","DJ113","DJ75","DJ61","DJ136","DJ86"),
                 "AsthmaHDMNeg"=c("DJ60","DJ125","DJ73","DJ134","DJ126"))
grouping <- stack(grouping) %>% rename(SAMPLE_ID=values,
                                       GroupUla=ind)

samples <- c("BAL", "BEC")
dataRoot <- here("Results", "Microarray_ImperialCollege")
sample <- samples[2]
for(sample in samples){
  message(sample)
  wdDir <- path(dataRoot, str_c(sample, "_", Sys.Date()))
  dir_create(wdDir)
  setwd(wdDir)
  phenoData <-
    read.AnnotatedDataFrame(here("Data", "Microarray_ImperialCollege",
                                 str_c(sample, "_expt_design_fixed.txt")))
  celfiles <- here("Data", "Microarray_ImperialCollege", sample)
  ...
  geoMeta <- bind_cols(pData(phenoData) %>% as_tibble(),
                       CEL_File = str_c(pData(phenoData)$EXPT, ".CEL")) %>%
    mutate(prettyName = str_c(GT, SAMPLE_ID, sep = "-"))
  write_xlsx(geoMeta, here("Notes", "GEO", str_c("meta", sample, ".xlsx")))
  file_copy(here("Data", "Microarray_ImperialCollege", sample, geoMeta$CEL_File),
            here("Notes", "GEO"))
  ...
  ### PCA with all probes
  expValue <- exprs(eset)
  allGroups <- list("Healthy"="Healthy", "AsthmaHDM"="AsthmaHDM", 
                    "AsthmaHDMNeg"="AsthmaHDMNeg", 
                    "All"=c("Healthy", "AsthmaHDM", "AsthmaHDMNeg"))
  ...
  contrasts <- makeContrasts(groupAsthmaHDM_DAY4 - groupAsthmaHDM_DAY14,
                             groupAsthmaHDMNeg_DAY4 - groupAsthmaHDMNeg_DAY14,
                             groupHealthy_DAY4 - groupHealthy_DAY14,
                             
                             groupAsthmaHDM_DAY14 - groupHealthy_DAY14,
                             groupAsthmaHDMNeg_DAY14 - groupHealthy_DAY14,
                             
                             groupAsthmaHDM_DAY4 - groupHealthy_DAY4,
                             groupAsthmaHDMNeg_DAY4 - groupHealthy_DAY4,
                             
                             (groupAsthmaHDM_DAY4 - groupAsthmaHDM_DAY14)-(groupHealthy_DAY4 - groupHealthy_DAY4),
                             (groupAsthmaHDMNeg_DAY4 - groupAsthmaHDMNeg_DAY14)-(groupHealthy_DAY4 - groupHealthy_DAY4),
                             (groupAsthmaHDM_DAY4 - groupAsthmaHDM_DAY14)-(groupAsthmaHDMNeg_DAY4 - groupAsthmaHDMNeg_DAY14),
                             levels=design)
