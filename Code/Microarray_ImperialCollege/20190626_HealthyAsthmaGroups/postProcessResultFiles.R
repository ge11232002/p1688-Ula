...
for(sample in samples){
  message(sample)
  wdDir <- file.path(dataRoot, paste0(sample, "_", "2019-07-08"), "resultsGenesets")
  dir.create(wdDir)
  setwd(wdDir)
  
  exprs <- read_tsv("../exprs_normalised_filtered.tsv")
  comparisonFns <- c("results_groupAsthmaAll_DAY14-groupHealthy_DAY14.tsv",
                     
                     "results_groupAsthmaAll_DAY4-groupHealthy_DAY4.tsv",
                     
                     "results_groupAsthmaAll_DAY4-groupAsthmaAll_DAY14.tsv",
                     "results_groupHealthy_DAY4-groupHealthy_DAY14.tsv",
                     
                     "results_(groupAsthmaAll_DAY4-groupAsthmaAll_DAY14)-(groupHealthy_DAY4-groupHealthy_DAY14).tsv"
  )
