...
for(sample in samples){
  message(sample)
  wdDir <- file.path(dataRoot, paste0(sample, "_", Sys.Date()), "VenDiagram")
  dir.create(wdDir)
  setwd(wdDir)

  comparisonFns <- c(AsthmaHDM="results_groupAsthmaHDM_DAY4-groupAsthmaHDM_DAY14.tsv",
                     AsthmaHDMNeg="results_groupAsthmaHDMNeg_DAY4-groupAsthmaHDMNeg_DAY14.tsv",
                     Healthy="results_groupHealthy_DAY4-groupHealthy_DAY14.tsv")
  ans <- list()
  i <- 1
